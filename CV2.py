...
# #Apply the Gaussina blur
# def blur(image):
#     return cv2.GaussianBlur(image, (15,15), 0)
 
# #Apply The binary thrusholding
# def threshold(image):
#     gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
#     _, thresh = cv2.threshold(gray, 127, 255, cv2.THRESH_BINARY)
#     return thresh
...
#     st.title(" Image Processing App")
#     st.write("Upload an Image and apply verious techinques on image")
...
#         #Select an operation
#         operation = st.selectbox("Choose an Operation", ['Original', 'Face Detection', 'Edge Detection', 'Grayscale', 'Blur', 'Threshold', 'Contour Detection', 'Sharpen', 'Emboss', 'Cartoonize', 'Invert'])
 
#         #Ensure the image is uploaded before processing
#         if uploaded_file:
#             image = load_image(uploaded_file)
